...
from cluster_monitor.presentation.controllers.epaper_controller import EpaperController
...
from cluster_monitor.shared.constants import RESOURCES_DIR
...
class EpaperRenderer(AbstractRenderer):
    def __init__(self, context: ContextEntity):
        ...
        self.font_type = ImageFont.truetype(os.path.join(RESOURCES_DIR, "Font.ttc"), DEFAULT_FONT_SIZE)
    ...
    def hard_refresh(self, skip_sleep: bool = False) -> None:
        if not skip_sleep:
            self.epd.sleep()
        self.epd.init()
        self.epd.Clear()
    ...
    def _run_periodic_init_task(self) -> None:
    ...
    def draw_text(
        ...
        text_width = self.draw.textlength(text, font=self.font_type)
        ...
        self.draw.text((text_x, text_y), text, font=self.font_type, fill=COLOR_BLACK)
    ...
    def draw_new_section(self, prev_coords: tuple[int, int, int, int] = NULL_COORDS) -> tuple[int, int, int, int]:
        ...
        line_y = y2 + DEFAULT_SECTION_Y_PADDING
    ...
    def draw_loading(self, prev_coords: tuple[int, int, int, int]) -> None:
        _, _, _, y2 = prev_coords
        text_width = self.draw.textlength("Loading...", font=self.font_type)
        center_y = y2 + (self.epd.width - y2) // 2
        center_x = (self.epd.height - text_width) // 2
        self.draw.text((center_x, center_y), "Loading...", font=self.font_type, fill=COLOR_BLACK)

    def _draw_apply(self, updated_area: tuple[int, int, int, int]) -> None:
    ...
    def draw_paragraph(
        ...
        for string in strings:
            ...
            text_width = self.draw.textlength(tentative_line, font=self.font_type)
    ...
    def get_current_page(self) -> int:
        return self.controller.get_current_page()

    def get_current_scroll_offset(self) -> int:
        return self.controller.get_current_scroll_offset()

    def get_current_scroll_step(self) -> int:
        return self.controller.get_current_scroll_step()

    def get_total_pages(self) -> int:
        return self.controller.get_total_pages()
    ...
    def draw_table(
        ...
        data: list[dict],
        prev_coords: tuple[int, int, int, int],
        font_size: int = DEFAULT_TABLE_FONT_SIZE,
    ) -> tuple[int, int, int, int]:
        ...
        current_y = start_y + header_border_width
        ...
        for index, (_, header_text) in enumerate(headers.items()):
            x = DEFAULT_SECTION_X_PADDING + (index * column_width)
        ...
        self.draw.line(
            (DEFAULT_SECTION_X_PADDING, current_y, self.epd.height - DEFAULT_SECTION_X_PADDING, current_y),
            fill=COLOR_BLACK,
        ...
        for row in data:
            ...
            for index, (key, _) in enumerate(headers.items()):
                x = DEFAULT_SECTION_X_PADDING + (index * column_width)
            ...
            self.draw.line(
                (DEFAULT_SECTION_X_PADDING, current_y, self.epd.height - DEFAULT_SECTION_X_PADDING, current_y),
                fill=COLOR_BLACK,
                width=internal_border_width,
            )

        return DEFAULT_SECTION_X_PADDING, start_y, self.epd.height - DEFAULT_SECTION_X_PADDING, current_y
